...
- `produce_document` tool now exposes PDF as the only polished public export; DOCX/XLSX/PPTX/HTML renderers remain parked legacy code until their own S-tier passes. PDF uses markdown → styled HTML/CSS → WeasyPrint, with pandoc/XeLaTeX fallback; argv-only subprocess, sanitizers, timeouts, 50MB cap
- Parked Marp themes: `default`/`gaia`/`uncover`/`thmanyah` remain in legacy renderer code but are hidden from product export surfaces
...
- **Extension productionization (2026-06-07, branch `spec/agent-browser-k8s-backend`): v1.0.0, Chrome Web Store-ready.** Full adversarial security review (the first whole-extension review) found and fixed: **real per-tab consent** (was advertised-but-unimplemented — now enforced on every tab-touching path via `chrome.storage.session` consent set + on-demand `scripting.executeScript` injection); **dropped the all-URLs declarative content script** for least-privilege (`permissions: activeTab/scripting/storage` only, no host perms, no WAR, explicit CSP, `tabs` dropped); **sender validation** on both `onMessage` listeners; **navigate SSRF allowlist** (scheme + RFC1918/loopback/link-local/metadata + IPv4-mapped-IPv6/trailing-dot parity with `url_sanitize.zig`); **latched STOP** (in-flight-cancelling, no auto-reconnect while killed); **per-command timeouts**; sensitive-field (password/cc) write guard; WS inbound size cap; `docs/PRIVACY.md`. Promoted `.spike/nullalis-extension` → **`clients/extension/`** (first-class, 35 renames, history preserved). Store submission package in `clients/extension/store/` (LISTING.md, SUBMISSION.md, permission justifications, 440×280 promo tile). **118 vitest pass**, typecheck/build/package green. Residual = human-only: CWS dev account, real screenshots, host the privacy policy, click submit.
