...
| D14 | Two pre-existing postgres test failures in the scheduler/jobs subsystem, inherited from `main` on the D8 baseline: `zaki_state.test.postgres claimed recurring job reschedules and records run` and `zaki_state.test.postgres replaceJobsJson scopes stored job ids per user while preserving raw ids`. Deterministic (fail on every `zig build test -Dengines=all` run), not caused by D8 or D11, and outside the vault code path. | Sprint 4 (silent-catch-sweep + test cleanup) | Tracked so D11 reviewers don't mistake them for regressions — the D11 tests add zero new failures on top of this baseline. Elevate if CI trust rot from the persistent red becomes an issue before Sprint 4. |
