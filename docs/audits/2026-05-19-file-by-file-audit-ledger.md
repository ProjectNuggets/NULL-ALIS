...
| B8 | MED | Coverage audit is missing. | No `.spike/coverage/<ts>/` active report. | v1.14.18 | J | CLOSED 9e008c22 — static-analysis test-reference audit landed: `.spike/coverage/run.sh` enumerates every `pub fn` in `src/`, matches each name as a token against the test corpus (in-file `test "..."` blocks + every file under `tests/`), emits `tested_pub_fns.txt` / `untested_pub_fns.txt` + a summary with per-file untested-count top-10. First baseline at `.spike/coverage/20260522T214515Z/` — 2633 pub fn declarations, 1665 unique names, 52.3% tested-by-name, 795 untested (top concentration: `zaki_state.zig` 193). Documented in `.spike/coverage/README.md` (one-off cadence; not a CI gate). Real LLVM line coverage deferred D50. |
