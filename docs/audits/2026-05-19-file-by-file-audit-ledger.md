...
| V6 | MED | Legacy `state.zig` deprecation/migration path needs explicit handling. | `docs/ROADMAP.md` v1.14.18 Step 8. | v1.14.18 | E | CLOSED — DELETE. Audit step 1 returned zero production callers; the prescriptive deprecation steps were predicated on live callers, so the §14.6 outcome was delete-not-deprecate (commit pending in this sprint). |
