...
| D44 | Clean K2.6 LoCoMo re-bench — a genuine multi-session / over-window test | the conv-0 ~94% was long-context recall on a then-dead memory pipeline; a clean number needs a test that exceeds the model window so the memory engine is actually exercised | v1.15.0 bench-iteration block | **in progress (2026-05-22)** — confirmed by active monitoring of a subset run: a single LoCoMo conversation (~30K tokens) fits entirely inside K2.6's 262K window, so compaction never fires (0 events, context pressure 12–18%), extraction is never triggered at a boundary, and the agent answers from the in-context transcript ("Based on Session 3…") — the bench measured long-context recall, NOT the memory engine. Fix landed: a **bench-only** env-gated context-window cap (`NULLALIS_BENCH_CONTEXT_LIMIT`) feeds `token_limit_override` so conversations overflow → compaction → extraction → retrieval; inert/absent in production. Re-bench runs the 2-arm disjoint-halves subset with the cap (~24–32K). |
