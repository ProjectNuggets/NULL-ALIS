...
| ID | Shape | Why deferred | Target | Status |
|----|-------|--------------|--------|--------|
| D52 | **PII storage policy** — the agent's reflex (LLM RLHF, not nullalis code) is to refuse persisting user-volunteered PII (phone numbers, addresses, contact info). Surfaced in long-conv QA S14: user asked "remember my brother Karim, +49 30 12345"; agent declined to call `memory_store`. Recommended Hybrid policy: (a) DEFAULT-store via a system-prompt directive that overrides the reflex for the user's OWN personal-memory channel, (b) TAG memories carrying detectable PII with `pii_tags=[phone,address,…]`, (c) ACKNOWLEDGE in the reply ("Stored — marked PII — say `forget pii` to clear"), (d) CONTROL via a `memory_purge_pii` tool, (e) PROTECT pii_tagged rows via at-rest encryption once the secret-vault (D47) integrates | Real product decision Nova wants to think about — refusing the user's own PII in a personal-memory product is paternalistic + breaks trust, but storing everything with no marking/control is reckless. Recommendation captured in chat; not implementing without explicit direction. Pillar 1 (prompt directive) is the smallest commercial-v1 fix; tagging/purge/encryption are V1.x follow-ups | v1.1 or earlier opportunistic | **open — design captured, awaiting Nova green-light** |
| D53 | **`<tool_call>` XML leak persists at ~7% intermittent rate** — across QA1 (T6), QA2 (T4a + T6), QA3 (T4a + T6), QA4 (T4a + T6), long-conv QA (3/40 ingest + S15 mid-stress). Multiple fix attempts landed: streaming hold widened to catch `<tool_call>` / `tool_call>` / `ool_call>` prefixes (`c6d9b8ea` + `83168ace`), defensive `stripToolCallMarkup` at safe_display_text boundary (`8cd90a0b`). All cosmetic — the reply text that follows the leaked fragment is correct and useful (e.g., T6: "ool_call>\n\nThe subagent (`sum_1_to_100`) completed successfully"). Replies that include the leak still answer the user's question | The actual chunk-flow that produces the leak hasn't been pinned — multiple iterations of the streaming-callback + flushValidatedReply path were re-read without finding the path that bypasses both the hold matcher and the strip helper. Likely needs a per-chunk debug log on `streamCallbackWithTiming` to trace WHICH chunk produces the leaked bytes, on which provider, under what iteration shape. Not blocking commercial v1 (the answer is correct), but visible in every tool-using turn at ~7% | next streaming-layer touch, or when a customer flags it | **open — known cosmetic, multiple partial fixes shipped** |
