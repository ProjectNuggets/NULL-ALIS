...
Last audit: **2026-05-22** at v1.14.18 (Audit MED-tier sweep) close — added section "From v1.14.18 — Audit MED-tier sweep" with **D48** (snapshot bi-temporal `valid_to` export/import asymmetry — parked V1.6 dependency, not a live bug), **D49** (V7 tail — rename `zaki_dual.zig`→`zaki_postgres.zig` + mirror-latency health metric + CLI flag alias; the load-bearing default-off contract shipped, polish deferred), and **D50** (B8 tail — real LLVM source-instrumented coverage; today's static-analysis pass `.spike/coverage/run.sh` surfaces the 795-fn actionable backlog, LLVM line coverage deferred for the next test-infra touch). Prior audit: 2026-05-22 at Sprint 3 (Universal API Connector) close — added section "From Sprint 3 — Universal API Connector" with **D47** (OpenAPI connector ships with env-var credential resolution; full secret-vault integration deferred). Prior audit: 2026-05-22 at v1.14.20 + Slice 2 close — added section "From v1.14.19 / v1.14.20" with **D36–D46** (11 new rows) covering the Sprint 2 / MCP V1.1 / Slice 2 deferrals: `curlPost` status threading, MCP HTTP-SSE / `resources` / `sampling`, Email IMAP IDLE, the `imapResponseComplete` perf walk, the irc/websocket unverified-TLS gap (D42 → task #22, planned), Nostr, the clean K2.6 re-bench, Sprint-2 cosmetic nits, and the config-plane index pointer. Every MAJOR review finding from Sprint 2 + Slice 2 was fixed before merge; only true deferrals are registered. Prior audit: 2026-04-26 (D28-Day-1 close + S9 park).
