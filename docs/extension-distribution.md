...
**Source of truth for the Web Store path:** `clients/extension/store/`.

- `clients/extension/store/LISTING.md` — every CWS dashboard field (summary
  ≤132 chars, detailed description, category, single-purpose statement,
  per-permission justifications, and the Privacy-practices data-usage
  disclosures mapped to `clients/extension/docs/PRIVACY.md`).
- `clients/extension/store/SUBMISSION.md` — the step-by-step submission runbook
  (developer account, build the zip, upload, fill the listing, assets, privacy
  form, submit, update cadence).
- `clients/extension/store/assets/` — the 440×280 promo tile
  (`promo-440x280.png`, generated by `scripts/gen-store-assets.sh`) and the
  screenshot capture checklist (`SCREENSHOTS.md`).

The in-repo `manifest.json` is now **version `1.0.0`** (first public release)
and store-compliant: least-privilege permissions (`activeTab`, `scripting`,
`storage`), an explicit locked-down CSP, icons at 16/48/128, and
**no `update_url`** — the Web Store supplies updates; `update_url` is added only
for the self-hosted managed `.crx` path in §2b. The upload artifact has
`manifest.json` at the **zip root** (`npm run package` zips the *contents* of
`dist/`), which CWS requires.
