...
Automated (green): the extension client builds (`.spike/nullalis-extension`, 51 vitest tests pass) and the gateway side passes (`extension_ws` hub + `extension_*` tools + the SSRF sanitizer).
```bash
cd .spike/nullalis-extension && npm install && npm run build && npm test   # extension client
