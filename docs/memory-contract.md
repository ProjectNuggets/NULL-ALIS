...
| Keys `history/<key>/<ts>-<ns>` (M2 edit supersede) | bookkeeping | born-closed snapshot of a SUPERSEDED memory version written by `Manager.editMemorySupersede`; `<ts>` is the edit's wall-clock second (`valid_at=valid_to=invalid_at=expired_at=<ts>`), `<ns>` a zero-padded sub-second nanosecond component so same-second edits keep DISTINCT snapshots; `is_latest=FALSE` so it is validity-filtered out of every read; append-only (`isAppendOnlyMemoryKey`), not embedded (`isSemanticBookkeepingKey`=true→`shouldEmbedMemoryEntry`=false), hidden from /brain (`BRAIN_HIDDEN_PREFIXES`). The live key keeps stable identity + injection; audit via `getMemoryAnyValidity` + the `edit_supersede` `memory_events` row (the tool claims "archived" only when the snapshot row actually landed) |
| `durable_fact/*` keys | knowledge | curable (archive/forget/**edit now supersedes with a `history/` row** — H1/M2), system-write-disciplined. **Archive/forget are information-scoped (M3)**: closing a key auto-closes every other live row with the identical SHA-256 `content_hash` (archive → bi-temporal close-out cascade; forget → hard-delete cascade, reaching autosave/behavior copies), and near-duplicates (different hash, matched via the content's salient token — longest ≥5-byte word, first-occurrence tie-break) are REPORTED in the tool result for follow-up curation, never auto-closed. Prevention side: the learning fast path skips a `durable_fact/behavior/*` write when a live row already holds byte-identical content (`findMemoryByContentHash`), keeping one canonical row per fact |
