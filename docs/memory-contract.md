...
| Keys `history/<key>/<ts>` (M2 edit supersede) | bookkeeping | born-closed snapshot of a SUPERSEDED memory version written by `Manager.editMemorySupersede`; `valid_at=valid_to=invalid_at=expired_at=<ts>`, `is_latest=FALSE` so it is validity-filtered out of every read; append-only (`isAppendOnlyMemoryKey`), not embedded (`isSemanticBookkeepingKey`=false→`shouldEmbedMemoryEntry`=false), hidden from /brain (`BRAIN_HIDDEN_PREFIXES`). The live key keeps stable identity + injection; audit via `getMemoryAnyValidity` + the `edit_supersede` `memory_events` row |
| `durable_fact/*` keys | knowledge | curable (archive/forget/**edit now supersedes with a `history/` row** — H1/M2), system-write-disciplined |
