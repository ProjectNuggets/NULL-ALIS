...
| Extracted predicate `NO_CONNECTION_FOUND`, `DESCRIPTION` | knowledge (fact only) | durable fact may be retained for audit/curation, but endpoint resolution and entity/edge materialization are suppressed (non-relational sentinel output, not graph knowledge) |
