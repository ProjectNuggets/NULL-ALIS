...
| Conversational meta-predicate (`GREETED`, `SAID`, `MENTIONED`, …) | bookkeeping | fully rejected: no memory, entity, edge, embedding, or recall candidate |
| Extracted predicate `NO_CONNECTION_FOUND`, `DESCRIPTION` | bookkeeping (audit only) | may be retained under `audit_shell/extraction_sentinel/*` for operator curation; hidden from Brain/default recall, not embedded, and excluded from endpoint/entity/edge materialization and contradiction judging |
...
- Extraction predicate policy: `src/agent/extraction_persist.zig` `REJECTED_PREDICATES` / `isRejectedPredicate` (full rejection) and `AUDIT_ONLY_PREDICATES` / `isAuditOnlyPredicate` (internal audit retention)
