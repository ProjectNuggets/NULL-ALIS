...
| `nullalis_artifact_export_total` | counter | `format`, `result` | YES | Artifact-export ops. Format ∈ {pdf, invalid}. DOCX/PPTX/XLSX/HTML are parked and counted as invalid-format rejections. Result ∈ {ok, invalid_input, invalid_format, missing_artifact, state_unavailable, state_error, internal_error, renderer_unavailable, export_failed}. `state_unavailable`=PG not configured; `state_error`=PG configured but read failed; `internal_error`=arg-map/body-build OOM or workspace config drift; `renderer_unavailable`=renderer binary missing (502); `export_failed`=renderer ran but failed on content (500). (cross_user_denied unreachable today — getArtifactById collapses "not owned" and "doesn't exist" into not-found.) |
...
| `nullalis_produce_document_total` | counter | `format`, `result` | NO | produce_document tool dispatch. Public success format is `pdf`; parked DOCX/PPTX/XLSX/HTML direct calls are invalid_input until their renderer passes reopen. Result ∈ {ok, tool_missing, render_failed, invalid_input}. |
