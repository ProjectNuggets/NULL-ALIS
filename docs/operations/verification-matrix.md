...
| 8 of 19 cascade tables (messages, completion_events, memory_events, tasks, job_runs, telegram_updates, channel_identity_bindings, tenant_user_leases) | These tables have no public seed CRUD on `zaki_state.Manager`, so the live D25 test can't reach them programmatically. | Static line-scan pins the CASCADE declaration on each. The 11 publicly-reachable tables are runtime-verified (`users` source + `user_config`, `heartbeat`, `channel_state`, `onboarding`, `sessions`, `memories`, `working_memory`, `user_secrets`, `secret_mutations`, `jobs`, `artifacts` + `artifact_versions` transitive, `trace_shares`). |
