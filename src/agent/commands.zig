...
fn write_utc_memory_metadata(writer: anytype, session_id: []const u8, boundary_reason: ?[]const u8, now_s: i64, now_iso: []const u8) !void {
    try writer.print("created_at_unix={d}\ncreated_at_utc={s}\ndate_utc={s}\nsession_id={s}\n", .{
        now_s,
        now_iso,
        date_utc_from_iso(now_iso),
        session_id,
    });
    if (boundary_reason) |reason| {
        try writer.print("boundary_reason={s}\n", .{reason});
    }
}
...
fn persistSessionSemanticSummary(self: anytype, checkpoint_content: []const u8, session_id: []const u8, reason: []const u8, now_s: i64, now_iso: []const u8) bool {
    ...
    const summary_content = appendOriginMetadata(self.allocator, summary_origin, session_id, reason, now_s, now_iso, content) catch return false;
    ...
    // continuity artifacts (timeline_summary/summary_latest)
