...
fn buildSafetySection(w: anytype) !void {
    ...
    try w.writeAll("- Capability walls and wishes: when a needed tool, skill, or permission doesn't exist and blocks the user's request, file a wish via `memory_store` with key `wish/<short-slug>` and content describing what was needed; include `evidence_run_id=<run_id>` if known. FIRE the memory_store call in the SAME turn — saying \"wish filed\" without the actual tool call is a fabricated promise (the tool log will show the mismatch). Tell the user a wish was filed only after the call succeeds. Wishes are proposals to the roadmap — the team will review them, not auto-build them. If the operator has enabled Decision Hub matching, `/learn list` re-checks open wishes and surfaces an install suggestion when a matching skill exists — but never promise automatic matching. Never claim the capability exists.\n\n");
...
test "safety section teaches wish filing at capability walls (bucket 5 proposals)" {
    ...
    // No new ## heading introduced by this bullet.
    try std.testing.expect(std.mem.indexOf(u8, output, "## Wish") == null);
    // Package 2b: the conduct closes the loop — a filed wish is re-matched
    // against the Decision Hub at /learn list time (render-time matchmaking).
    try std.testing.expect(std.mem.indexOf(u8, output, "/learn list") != null);
    try std.testing.expect(std.mem.indexOf(u8, output, "Decision Hub") != null);
    // Wish-matchmaking round 2, Finding 2 (honest-prompt rule): matchmaking is
    // operator-gated (default OFF), runs only at /learn list time, and caps at
    // 5 wishes — so the bullet must not promise it unconditionally. The wording
    // is conditional-truthful in BOTH flag states and forbids promising
    // automatic matching.
    try std.testing.expect(std.mem.indexOf(u8, output, "If the operator has enabled Decision Hub matching") != null);
    try std.testing.expect(std.mem.indexOf(u8, output, "never promise automatic matching") != null);
    // The old over-promise ("will be matched automatically as the catalog
    // grows") must be gone — it lied whenever the flag was off.
    try std.testing.expect(std.mem.indexOf(u8, output, "matched automatically") == null);
    try std.testing.expect(std.mem.indexOf(u8, output, "reassure the user") == null);
}
