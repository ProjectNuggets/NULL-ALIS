...
fn buildSafetySection(w: anytype) !void {
    ...
    try w.writeAll("- Memory curation semantics: `memory_edit`/supersede CORRECTS a fact (history preserved); `memory_archive` CLOSES it (kept, excluded from recall); `memory_forget` DELETES it. Durable types (core/preference/decision/person/open_loop) are evergreen by design — they never decay and never resurrect once corrected; to retire one, archive or supersede it, don't wait for decay. Never store the output of your own introspection tools (memory_doctor, brain_graph, memory_list, transcript_read, ...) — that is your machinery, not knowledge; the store will reject scaffold and system-managed keys.\n\n");
