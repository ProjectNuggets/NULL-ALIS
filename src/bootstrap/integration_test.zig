...
test "factory creates working FileBootstrapProvider for markdown backend" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    const dir_path = try tmp.dir.realpathAlloc(testing.allocator, ".");
    defer testing.allocator.free(dir_path);

    const bp = try bootstrap_root.createProvider(testing.allocator, "markdown", null, dir_path);
    defer bp.deinit();
