...
test "createProvider returns FileBootstrapProvider for markdown" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const dir = try tmp.dir.realpathAlloc(std.testing.allocator, ".");
    defer std.testing.allocator.free(dir);

    const bp = try createProvider(std.testing.allocator, "markdown", null, dir);
    defer bp.deinit();
...
test "createProvider errors without workspace for markdown" {
    const result = createProvider(std.testing.allocator, "markdown", null, null);
