...
pub const SignalChannel = struct {
    ...
    pub fn sendTypingIndicator(self: *SignalChannel, target_str: []const u8) void {
        ...
        const resp = root.http_util.curlPost(TYPING_ALLOCATOR, url, rpc_body, &.{}) catch return;
    ...
    pub fn startTyping(self: *SignalChannel, target: []const u8) !void {
        ...
        const key_copy = try TYPING_ALLOCATOR.dupe(u8, target);
...
test "prepare outgoing content keeps unresolved relative attachment path" {
    ...
    const marker_text = "[IMAGE:nullalis_nonexistent_attachment_123456789.png]";
    ...
    try std.testing.expectEqualStrings("nullalis_nonexistent_attachment_123456789.png", prepared.attachments[0]);
}
