        ...
        if (mem == .object) {
            ...
            if (mem.object.get("lifecycle")) |lc_val| {
                if (lc_val == .object) {
                    ...
                    if (lc.get("tool_traces_retention_days")) |v| if (v == .integer and v.integer >= 0 and v.integer <= std.math.maxInt(i32)) {
                        self.memory.lifecycle.tool_traces_retention_days = @intCast(v.integer);
                    };
                    if (lc.get("subagent_results_retention_days")) |v| if (v == .integer and v.integer >= 0 and v.integer <= std.math.maxInt(i32)) {
                        self.memory.lifecycle.subagent_results_retention_days = @intCast(v.integer);
                    };
                    if (lc.get("memory_events_retention_days")) |v| if (v == .integer and v.integer >= 0 and v.integer <= std.math.maxInt(i32)) {
