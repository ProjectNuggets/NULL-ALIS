...
pub const link_types = @import("link_types.zig");
pub const LinkType = link_types.LinkType;
pub const ALL_LINK_TYPES = link_types.ALL_LINK_TYPES;
...
/// Fail-closed write-boundary backstop for internal assistant context fences.
/// These byte signatures are generated by the runtime itself and are never
/// valid durable memory content. Matching is ASCII case-insensitive and the
/// ZAKI envelope accepts the whitespace tolerated by the Hub sanitizer.
pub fn containsAssistantScaffold(content: []const u8) bool {
    if (containsAssistantScaffoldTag(content, "memory_for_turn")) return true;
    if (containsAssistantScaffoldTag(content, "memory_context")) return true;

    var offset: usize = 0;
    while (std.mem.indexOfPos(u8, content, offset, "[[")) |start| {
        var i = start + 2;
        ...
        const prefix = "ZAKI_";
        if (i + prefix.len <= content.len and std.ascii.eqlIgnoreCase(content[i .. i + prefix.len], prefix)) {
            return true;
        }
        offset = start + 2;
    }
...
fn containsAssistantScaffoldTag(content: []const u8, tag_name: []const u8) bool {
    var offset: usize = 0;
    while (std.mem.indexOfPos(u8, content, offset, "<")) |start| {
        var i = start + 1;
        while (i < content.len and std.ascii.isWhitespace(content[i])) : (i += 1) {}
        ...
        if (i + tag_name.len <= content.len and std.ascii.eqlIgnoreCase(content[i .. i + tag_name.len], tag_name)) {
            const end = i + tag_name.len;
            if (end == content.len or std.ascii.isWhitespace(content[end]) or content[end] == '>' or content[end] == '/') {
                return true;
            }
        }
        offset = start + 1;
...
pub const Memory = struct {
    ...
    pub fn store(self: Memory, key: []const u8, content: []const u8, category: MemoryCategory, session_id: ?[]const u8) !void {
        if (containsAssistantScaffold(content)) return error.AssistantScaffoldRejected;
...
test "Memory facade rejects assistant scaffold for plain and metadata stores" {
    var backend = none.NoneMemory.init();
    defer backend.deinit();
    const m = backend.memory();
    ...
    try std.testing.expectError(
        ...
        m.store("key", "fact <memory_for_turn>private fuel</memory_for_turn>", .core, null),
    );
    try std.testing.expectError(
        error.AssistantScaffoldRejected,
        m.storeWithMetadata(
            "key",
            "fact [[ZAKI_MEMORY_CONTEXT_V2]]private fuel",
            .core,
            null,
            "{}",
        ),
