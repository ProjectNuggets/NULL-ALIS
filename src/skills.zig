...
// skills/delegation/skill.json is a valid manifest AND that
// listSkillsMerged actually discovers it — not just that the JSON parses
// in isolation.
test "listSkillsMerged discovers the shipped delegation skill" {
    const allocator = std.testing.allocator;
    ...
    // skills/delegation/ directory, and `zig build test` runs with cwd at the
    // repo root (verified: build.zig does not override addRunArtifact's cwd).
    const skills = try listSkillsMerged(allocator, ".", "/tmp/nullalis-nonexistent-workspace-delegation-test");
    defer freeSkills(allocator, skills);

    var found: ?Skill = null;
    for (skills) |s| {
        if (std.mem.eql(u8, s.name, "delegation")) found = s;
    }
    const skill = found orelse return error.TestUnexpectedResult;

    try std.testing.expectEqualStrings("delegation", skill.name);
